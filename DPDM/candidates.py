...
if __name__ == '__main__':
    ...
    # as well as list of successful frequencies and predicted epsilon's
    count = 0
    cand_freqs = []
    cand_p0s = []
    cand_sigmas = []
    ...
    for (coh, (lof, hif_inclusive, df)) in zip(coherence_times[start_ind:end_ind + 1],
        ...
        if CHECK_CANDIDATES and len(freq_inds) > 0:
            ...
            for i in np.where(pfull > 0.01)[0]:
                ...
                # Compute and record predicted epsilon and error bar
                int_grid, cdf = calculate_cdf(s[freq_ind], z[freq_ind])
                eps = (int_grid[1:] + int_grid[:-1]) / 2
                deps = int_grid[1:] - int_grid[:-1]
                pdf = (cdf[1:] - cdf[:-1]) / deps
                mean = np.sum(pdf * eps * deps)
                std = np.sqrt(np.sum(pdf * eps ** 2 * deps) - mean ** 2)
                succ_epsmeans.append(mean)
                succ_epsstds.append(std)
                ...
                if VERBOSE:
                    ...
                    print(f'epsilon = {mean:.1e} +- {std:.1e}')
                    sys.stdout.flush()
    ...
    if CHECK_CANDIDATES:
        ...
        np.savez(f'candidates/{filename}',
                 ...
                 succ_epsmeans = succ_epsmeans,
                 succ_epsstds = succ_epsstds)
